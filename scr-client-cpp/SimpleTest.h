...
class SimpleTest : public WrapperBaseDriver
{
public:
	
	// Constructor
	SimpleTest(){stuck=0;clutch=0.0;};

    std::array<float, 19> getTrack(CarState cs);
...
private:
    ...
    float getAngle(CarState cs);

    float getDamage(CarState cs);

    float getDistFromStart(CarState cs);

    float getDistRaced(CarState cs);

    float getFocus(CarState cs);

    float getWheelSpinVelocity(CarState cs, int wheel);

    // float getTrack(CarState cs, int i);
